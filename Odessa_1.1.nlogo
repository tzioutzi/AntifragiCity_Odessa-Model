...
breed [ responders resp ]                ; emergency responders that support the evacuation procedures of the vulnerable population
...
residents-own [  ; the variables that residents own
  ...
  evacuated?     ; true if an agent is evacuated
  dead?          ; true if an agent is dead
...
responders-own [
  ...
  moving?        ; true if the agent is moving, false if not (e.g., starting from evacuation point)
...
globals [        ; global variables
  ...
  evacuation_target
                ; agents (residents, pedestrians or cars) that can be targets of the evacuation procedures
  followers     ; agents (residents, pedestrians or cars) collected by responders to evacuate faster
...
to move-gm
  set car_ahead cars in-cone (45 / patch_to_meter) 20                                        ; get the cars ahead in 150ft (almost half a block) and in field of view of 20 degrees
...
to setup-init-val
  ...
  set R1_HorEvac_Foot 63          ; 63% of the agents evacuate horizontally on foot
  set R2_HorEvac_Car 27           ; 27% of the agents evacuate horizontally with their car
  set R3_VerEvac_Foot 63          ; 63% of the agents evacuate on foot and are open to vertical evaucation if it is closer to them compared to a shelter outside the inundation zone
  set R4_VerEvac_Car 27           ; 27% of the agents evacuate with their car and are open to vertical evaucation if it is closer to them compared to a shelter outside the inundation zone
  ...
  set Ped_Sigma 0.65              ; the standard deviation of the normal dist. that the walking speed of the agents are drawn from is set to 0.20 m/s
  ...
  set Rsig3 1.65
  set Rsig4 1.65
  set vuln_speed 0.05             ; speed of vulnerable population (reduced mobility)
end
...
to read-gis-files
  gis:load-coordinate-system "road_network/road_network_fixed.prj"                                        ; load the projection system - WGS84 / UTM (METER) for your specific area
  set shelter_locations gis:load-dataset "shelter_locations/evacuation_points.shp"                        ; read shelter locations
  set road_network gis:load-dataset "road_network/road_network_fixed.shp"                                 ; read road network
  set population_distribution gis:load-dataset "population_distribution/shelterd_population.shp"          ; read population distribution
      ...
      foreach j [ k ->
        if length ( gis:location-of k ) = 2 [                          ; check if the vertex has both x and y
          ...
          create-residents 1 [                                         ; create the agent
            ...
            if speed < 0.001                                           ; if speed is too low, set it to very small non-zero value
                 [set speed 0.001]
            if speed < vuln_speed                                      ; if speed is low, define the resident as vulnerable
                 [set vulnerable? true]
...
to go
  ...
  if Evacuation_Mode = "NO"
      []

  if Evacuation_Mode = "0"
      [evacuate_vulnerable_0]

  if Evacuation_Mode = "1"
      [evacuate_vulnerable_1]

  if Evacuation_Mode = "2"
      [evacuate_vulnerable_2]

  if Evacuation_Mode = "3"
      [evacuate_vulnerable_3]
...
to-report per_evacuated
 ifelse any? (turtle-set residents pedestrians cars)
  [report count (turtle-set residents pedestrians cars) with [ evacuated? = true ] / (count residents + count pedestrians + count cars) * 100]
  [report "n/a"]

end

to-report vulnerable_evac_rate
  ifelse any? residents with [vulnerable? = true] or any? pedestrians with [vulnerable? = true] or any? cars with [vulnerable? = true]
   [report (count residents with [vulnerable? = true and evacuated? = true] +
            count pedestrians with [vulnerable? = true and evacuated? = true] +
            count cars with [vulnerable? = true and evacuated? = true])
            /(count residents with [vulnerable? = true] + count pedestrians with [vulnerable? = true] + count cars with [vulnerable? = true]) * 100]
   [report "n/a"]
...
to-report throughput
 ...
 ifelse any? (turtle-set pedestrians cars responders)
   [report (count (turtle-set pedestrians cars responders) with [moving?] / count (turtle-set residents pedestrians cars responders))]
   [report "n/a"]
end
...
to-report stress
 ...
 ifelse any? (turtle-set pedestrians cars)
  [report 0.5 * ((mean [speed] of pedestrians - Ped_Speed) / Ped_Speed) +
...
to-report redundancy
  ...
  ifelse any? intersections with [any? (turtle-set cars) with [moving?] in-radius 2]
...
to-report entropy
 ...
 ifelse any? (turtle-set pedestrians cars)
  [report -1 * (ln (mean [speed] of (turtle-set pedestrians cars))) / (ln (mean [max_speed] of (turtle-set pedestrians cars)))]
  [report "n/a"]
end
...
to evacuate_vulnerable_0

 ask intersections with [shelter?]                   ; initiation of response processes from the shelters
   [if count responders < 5 and not any? responders with [not is-full?] in-radius 6
     [hatch-responders 1
       [set color red
        set shape "square"
        set size 2
...
ask responders with [not is-full?]
    [if any? residents with [vulnerable? = true and evacuated? = false]
        [ifelse not any? residents with [collected?] in-radius 1
          [set evacuation_target max-one-of residents with [vulnerable? = true and collected? = false and evacuated? = false] [distance myself]]
          [set evacuation_target min-one-of residents with [vulnerable? = true and collected? = false and evacuated? = false] [distance myself]]
         if evacuation_target != nobody
          [set heading towards evacuation_target
           set moving? true
           fd max_speed / fd_to_kmph]]]

ask responders with [moving? = true and is-full? = false]
  [if evacuation_target != nobody
        [if distance evacuation_target < 1 ;with [ distance self < 1]               ; if close enough to pick up and evacuate
            [create-evac-to evacuation_target [tie ]
            fd 0]
             ask evacuation_target
              [set collected? true]

         set followers residents with [collected?]
         ask followers
              [move-to min-one-of responders [distance myself]]

         if count followers in-radius 1 > 10
            [set is-full? true
             set color black
             set heading towards min-one-of intersections with [shelter?] [distance myself]]

ask responders with [moving? and is-full?]
            [set heading towards min-one-of intersections with [shelter?] [distance myself]
             fd max_speed / fd_to_kmph
             if distance min-one-of intersections with [shelter?] [distance myself] < 1
                  [ask followers
                      [set evacuated? true]
                      die]]  ]]

  ask intersections with [shelter?]
    [ask residents in-radius 3
       [set evacuated? true
        die]]

ask responders                                              ; if no more evacuations of vulnerable population are required stop the evacuation processes
  [if not any? residents with [vulnerable? = true and evacuated? = false]
    [die]]

end

to evacuate_vulnerable_1
...
ask responders with [not is-full?]
    [if any? residents with [vulnerable? = true and evacuated? = false]
        [ifelse not any? residents with [collected?] in-radius 1
          [set evacuation_target one-of residents with [ speed = min [speed] of residents and vulnerable? = true and collected? = false and evacuated? = false]]
          [set evacuation_target one-of residents with [ speed = min [speed] of residents and vulnerable? = true and collected? = false and evacuated? = false]]
        if evacuation_target != nobody
          [set heading towards evacuation_target
           set moving? true
           set speed max_speed / fd_to_kmph
            fd speed]]]

ask responders with [moving? and not is-full?]
  [if evacuation_target != nobody
        [if distance evacuation_target < 1                 ; if close enough to pick up and evacuate
            [create-evac-to evacuation_target [tie ]
            fd 0]
             ask evacuation_target
              [set collected? true]
...
ask responders with [not is-full?]
    [if any? (turtle-set residents pedestrians) with [vulnerable? = true and evacuated? = false]
        [ifelse not any? residents with [collected?] in-radius 1
          [set evacuation_target one-of (turtle-set residents pedestrians) with [ speed = min [speed] of (turtle-set residents pedestrians) and vulnerable? = true and collected? = false and evacuated? = false]]
          [set evacuation_target one-of (turtle-set residents pedestrians) with [speed = min [speed] of (turtle-set residents pedestrians) and vulnerable? = true and collected? = false and evacuated? = false]]
        if evacuation_target != nobody
          [set heading towards evacuation_target
           set moving? true
...
to evacuate_vulnerable_3

  ask n-of 5 (turtle-set intersection 269 intersection 268 intersection 267 intersection 266 intersection 265 intersection 595 intersection 368 intersection  593 intersection 245 intersection 0 intersection 14)
                                                                                                   ; initiation of response processes from areas near the port
   [if count responders < 5 and not any? responders with [not is-full?] in-radius 6
     [hatch-responders 1
       [set color red
        set shape "square"
...
1305
113
Vulnerable Evacuated (%)
vulnerable_evac_rate
2
1
11
...
Minutes (after the evacuation signal)
#
0.0
10.0
0.0
...
1508
461
Vulnerable Population Evacuated
Min
%
0.0
10.0
...
"" ""
PENS
"Evacuated" 1.0 0 -10899396 true "" "plotxy (ticks / 60) (count residents with [ evacuated? = true and vulnerable? = true] \n+ count pedestrians with [ evacuated? = true and vulnerable? = true] \n+ count cars with [ evacuated? = true and vulnerable? = true]) \n/ (count residents with [vulnerable? = true] \n+ count pedestrians with [vulnerable? = true] \n+ count cars with [vulnerable? = true]) * 100"
"Not Evacuated" 1.0 0 -2674135 true "" "plotxy (ticks / 60) (count residents with [ evacuated? = false and vulnerable? = true]\n+ count pedestrians with [ evacuated? = false and vulnerable? = true] \n+ count cars with [ evacuated? = false and vulnerable? = true]) \n/ (count residents with [vulnerable? = true] \n+ count pedestrians with [vulnerable? = true] \n+ count cars with [vulnerable? = true]) * 100"
"Collected" 1.0 0 -1184463 true "" "plotxy (ticks / 60) (count residents with [ collected? = true and vulnerable? = true]\n+ count pedestrians with [ collected? = true and vulnerable? = true] \n+ count cars with [ collected? = true and vulnerable? = true]) \n/ (count residents with [vulnerable? = true] \n+ count pedestrians with [vulnerable? = true] \n+ count cars with [vulnerable? = true]) * 100"
...
228
524
Responses for vulnerable population:
11
0.0
1
...
read-all</setup>
  ...
  <experiment name="1H_Response NO" repetitions="5" runMetricsEveryStep="false">
    <timeLimit steps="3600"/>
    <metric>throughput</metric>
    <metric>stress</metric>
    <metric>redundancy</metric>
    <metric>entropy</metric>
    <metric>per_evacuated</metric>
    <metric>vulnerable_evac_rate</metric>
    <enumeratedValueSet variable="Rsig3">
      <value value="1.65"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="Rsig4">
      <value value="1.65"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="immediate_evacuation">
      <value value="true"/>
    </enumeratedValueSet>
    ...
    <enumeratedValueSet variable="R3_VerEvac_Foot">
      <value value="63"/>
    </enumeratedValueSet>
    ...
    <enumeratedValueSet variable="R4_VerEvac_Car">
      <value value="27"/>
    </enumeratedValueSet>
    ...
    <enumeratedValueSet variable="Rtau4">
      <value value="10"/>
    </enumeratedValueSet>
    ...
    <enumeratedValueSet variable="R1_HorEvac_Foot">
      <value value="63"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="R2_HorEvac_Car">
      <value value="27"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="Evacuation_Mode">
      <value value="&quot;NO&quot;"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="vuln_speed">
      <value value="0.05"/>
    </enumeratedValueSet>
  ...
  <experiment name="1Η_Response 0" repetitions="5" runMetricsEveryStep="false">
    <timeLimit steps="3600"/>
    <metric>throughput</metric>
    <metric>stress</metric>
    <metric>redundancy</metric>
  ...
  <experiment name="Response 3" repetitions="5" runMetricsEveryStep="true">
    <setup>load1
load2</setup>
    <go>go</go>
  ...
  <experiment name="1Η_Response 2" repetitions="2" runMetricsEveryStep="false">
    <timeLimit steps="3600"/>
    <metric>throughput</metric>
    <metric>stress</metric>
    <metric>redundancy</metric>
  ...
  <experiment name="1Η_Response 1" repetitions="5" runMetricsEveryStep="false">
    <timeLimit steps="3600"/>
    <metric>throughput</metric>
    <metric>stress</metric>
    <metric>redundancy</metric>
  ...
  <experiment name="Response 2" repetitions="5" runMetricsEveryStep="true">
    <timeLimit steps="1800"/>
    <metric>throughput</metric>
    <metric>stress</metric>
    <metric>redundancy</metric>
...
load2</setup>
  ...
  <experiment name="1Η_Response 3" repetitions="5" runMetricsEveryStep="false">
    <timeLimit steps="3600"/>
    <metric>throughput</metric>
    <metric>stress</metric>
    <metric>redundancy</metric>
